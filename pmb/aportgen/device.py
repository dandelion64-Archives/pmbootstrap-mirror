...
def ask_for_architecture(logfd):
    ...
    while True:
        ret = pmb.helpers.cli.ask(logfd, "Device architecture", architectures,
...
def ask_for_manufacturer(logfd):
    logging.info("Who produced the device (e.g. LG)?")
    return pmb.helpers.cli.ask(logfd, "Manufacturer", None, None, False)


def ask_for_name(logfd, manufacturer):
    logging.info("What is the official name (e.g. Google Nexus 5)?")
    ret = pmb.helpers.cli.ask(logfd, "Name", None, None, False)
...
def ask_for_flash_method(logfd):
    while True:
        logging.info("Which flash method does the device support?")
        method = pmb.helpers.cli.ask(logfd, "Flash method",
            ...
            if method == "heimdall":
                ...
                while True:
                    ...
                    logging.info("<https://wiki.postmarketos.org/wiki"
                                 ...
                                 ".3F>")
                    heimdall_type = pmb.helpers.cli.ask(logfd, "Type",
                                                        heimdall_types,
                                                        heimdall_types[0])
...
def ask_for_bootimg(args):
    ...
    while True:
        response = pmb.helpers.cli.ask(args.logfd, "Path", None, "", False)
...
def generate(args, pkgname):
    arch = ask_for_architecture(args.logfd)
    manufacturer = ask_for_manufacturer(args.logfd)
    name = ask_for_name(args.logfd, manufacturer)
    year = ask_for_year(args.logfd)
    chassis = ask_for_chassis(args.logfd)
    has_keyboard = ask_for_keyboard(args)
    has_external_storage = ask_for_external_storage(args)
    flash_method = ask_for_flash_method(args.logfd)
    bootimg = None
    if flash_method in ["fastboot", "heimdall-bootimg"]:
