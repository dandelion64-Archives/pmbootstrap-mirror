...
def replace_variable(apkbuild, value: str) -> str:
    def getKeyErrorLog(match):
        return f"{apkbuild['pkgname']}: key '{match.group(1)}' for replacing "
        f"'{match.group(0)}' not found, ignoring"
    ...
    for match in revar.finditer(value):
        try:
            ...
            logging.verbose(getKeyErrorLog(match))
